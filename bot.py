...
def admin_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("👤 Соҳибони аккаунтҳо", callback_data="admin:account_owners")],
        [InlineKeyboardButton("🔎 Ҷустуҷӯи корбар", callback_data="admin:user_search")],
...
async def handle_text(update, context):
    ...
    # ADMIN: search user by Telegram ID, then send a private message
    if is_admin(user.id) and state == "admin_user_search_id":
        raw = update.message.text.strip()
        if not raw.isdigit():
            await update.message.reply_text("❌ Telegram ID бояд танҳо рақам бошад.")
            return
        target_id = int(raw)
        target = get_user(target_id)
        if not target:
            await update.message.reply_text(
                "❌ Корбар дар база ёфт нашуд.\n\n"
                "Корбар бояд пештар бо бот /start карда бошад.",
        ...
        await update.message.reply_text(
            ...
            f"👤 Username: {e(uname)}\n"
            f"📛 Ном: {e(target['first_name'] or '—')} {e(target['last_name'] or '')}\n"
            f"💰 Баланс: <b>{money(target['balance'])} сомонӣ</b>\n\n"
            "✉️ Акнун матнеро нависед, ки ба ин корбар фиристода шавад:",
    ...
    if is_admin(user.id) and state == "admin_user_search_message":
        ...
        if not target_id:
            context.user_data.clear()
            await update.message.reply_text("❌ Сессия гузашт.", reply_markup=admin_menu())
            return
        if not text:
            await update.message.reply_text("❌ Матн холӣ буда наметавонад.")
            return
        try:
            await context.bot.send_message(
            ...
            context.user_data.clear()
            await update.message.reply_text(
                f"✅ Паём ба <code>{target_id}</code> фиристода шуд.",
                reply_markup=admin_menu(), parse_mode=ParseMode.HTML)
        except TelegramError as ex:
            context.user_data.clear()
            await update.message.reply_text(
                "❌ Паём фиристода нашуд. Эҳтимол корбар ботро block кардааст ё ҳеҷ гоҳ /start накардааст.\n\n"
                f"Тафсилот: <code>{e(str(ex))}</code>",
                reply_markup=admin_menu(), parse_mode=ParseMode.HTML)
        return
...
async def admin_callback(update, context):
    ...
    if data == "admin:user_search":
        await q.answer()
        context.user_data.clear()
        context.user_data["state"] = "admin_user_search_id"
        await q.message.edit_text(
            "🔎 <b>ҶУСТУҶӮИ КОРБАР</b>\n\n"
            "🆔 Telegram ID-и корбарро нависед:",
            reply_markup=admin_back(), parse_mode=ParseMode.HTML)
        return

    if data == "admin:account_owners":
        await q.answer()
        items = account_listings(None, 100)
        if not items:
            await q.message.edit_text(
                "👤 <b>СОҲИБОНИ АККАУНТҲО</b>\n\nҲоло аккаунте нест.",
                reply_markup=admin_back(), parse_mode=ParseMode.HTML)
            return
        lines = ["👤 <b>СОҲИБОНИ АККАУНТҲО</b>\n"]
        for a in items:
            uname = a.get("username") or "username надорад"
            status = account_status_label(a.get("status", ""))
            lines.append(
                f"📦 <b>Аккаунт #{a['id']}</b>\n"
                f"👤 {e(uname)}\n"
                f"🆔 Telegram ID: <code>{a['user_id']}</code>\n"
                f"🎮 FF ID: <code>{e(a.get('ff_id',''))}</code>\n"
                f"💰 Нарх: <b>{money(a['price'])} сомонӣ</b>\n"
                f"📌 Статус: {e(status)}\n")
        await q.message.edit_text(
            "\n".join(lines), reply_markup=admin_back(), parse_mode=ParseMode.HTML)
        return
